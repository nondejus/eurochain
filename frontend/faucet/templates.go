...
<head>
	<title>EUR Faucet</title>
</head>
<body>
	<div align="center">
		<h1 style="margin-top:3em">eurochain {{.ChainNetwork}} faucet</h1>
		...
		<h3>Request %[1]d EUR by entering your address below and submitting the form.</h3>
		<form action="/request/tokens" method="POST">
			<div>Address: <input type="text" size="78" name="uh"></div>
			<br>
			<div><input type="submit" value="Request %[1]d EUR" style="width:20em;height:2em;font-weight:bold;font-size:1em;"></div>
		</form>
		...
		<div style="margin-top:50px;"><small>eurochain faucet v0.1</small></div>
	</div>
</body>
`, coinsToGive))
...
<body>
	<div align="center">
		<h1>%d EUR succesfully transferred on eurochain's {{.ChainNetwork}} to {{.Address}}</h1>
...
<body>
	<div align="center">
		<h1>Succesfully {{.Action}} address {{.Address}} on eurochain"s {{.ChainNetwork}}</h1>
		<p>You can look up the transaction using the following ID:</p>
		<div><code>{{.TransactionID}}</code></div>
		<p><a href="/">Return to the homepage</a></p>
		<div style="margin-top:50px;"><small>eurochain faucet v0.1</small></div>
