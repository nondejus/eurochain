...
import (
	...
	"github.com/nbh-digital/eurochain/pkg/config"
	...
	go func() {
		...
		if cs != nil {
			...
			mintingPlugin = minting.NewMintingPlugin(
				...
				eurochaintypes.TransactionVersionMinterDefinition,
				eurochaintypes.TransactionVersionCoinCreation,
				&minting.PluginOptions{
					CoinDestructionTransactionVersion: eurochaintypes.TransactionVersionCoinDestruction,
				},
			)
			...
			authCoinTxPlugin = authcointx.NewPlugin(
				...
				eurochaintypes.TransactionVersionAuthAddressUpdate,
				eurochaintypes.TransactionVersionAuthConditionUpdate,
				nil, // no custom opts
			)
			// add the HTTP handlers for the auth coin tx extension as well
			if tpool != nil {
				authcointxapi.RegisterConsensusAuthCoinHTTPHandlers(
					...
					tpool, eurochaintypes.TransactionVersionAuthConditionUpdate,
					eurochaintypes.TransactionVersionAuthAddressUpdate)
			} else {
				authcointxapi.RegisterConsensusAuthCoinHTTPHandlers(
					router, authCoinTxPlugin,
					nil, eurochaintypes.TransactionVersionAuthConditionUpdate,
					eurochaintypes.TransactionVersionAuthAddressUpdate)
			}
