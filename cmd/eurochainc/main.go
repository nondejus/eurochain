...
import (
	...
	"github.com/nbh-digital/eurochain/pkg/types"
...
func main() {
	...
	cliClient, err := NewCommandLineClient("http://localhost:28110", bchainInfo.Name, daemon.RivineUserAgent)
	if err != nil {
		panic(err)
	}
	...
	cliClient.PreRunE = func(cfg *client.Config) (*client.Config, error) {
		...
		case config.NetworkNameTestnet:
			...
			cfg.GenesisBlockTimestamp = 1570710000 // timestamp of block #1
